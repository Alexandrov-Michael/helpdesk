...
class GetDepartamentForPcListView(LoginRequiredMixin, GetOdjectMixin, ListView):
    ...
    def get_queryset(self):
        ...
        try:
            ...
            raise Http404
        queryset = CompanyPC.objects.filter(company=com_user.company.id).select_related('departament__id', 'departament__name').order_by('departament.id').distinct('departament').order_by('deparament.name')
        return queryset
